...
def generate_layout_and_synthetic_images(
        ...
        pixels_size,
        synthetic_data_save_name = None,
        vpl_mode = "naive",
        vpg_mode = "HED",
    ):
    if synthetic_data_save_name is None:
    ...
    results = []
    for idx in tqdm(range(num_layouts)):
    ...
    for i in range(num_layouts):
        ...
        for j in range(num_samples_per_layout):
            imwrite(os.path.join(synthetic_sample_save_path, "syn%03d.jpg"%j), results[i][j])
...
def main():
    ...
    parser.add_argument("--synthetic_data_save_name", default=None, type=str)
    ...
    CUDA_VISIBLE_DEVICES=3 python3 3_generate_layout_and_synthetic_images.py \
    ...
    generate_layout_and_synthetic_images(
        ...
        synthetic_data_save_name=args.synthetic_data_save_name, 
